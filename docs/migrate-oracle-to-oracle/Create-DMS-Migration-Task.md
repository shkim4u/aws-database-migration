...
8. 이 시점에서 ```태스크```는 실행되며 소스 데이터베이스의 ```DMS_SAMPLE``` 스키카의 데이터를 ```Amazon RDS Oracle``` 인스턴스로 복제하기 시작합니다. ```DMS``` 작업은 ```생성 (상태=생성 중)```하는 데 잠시 시간이 걸린 다음 ```상태=Created```, ```상태=시작 중```, ```상태=실행 중```으로 전환됩니다. 새로 고침 버튼을 사용하여 작업 상태를 업데이트할 수 있습니다.
