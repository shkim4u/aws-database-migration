...
이제 브라우저에서 ```Application Load Balancer```의 ```DNS 이름```을 사용하여 ```/travelbuddy/hotelspecials``` 경로와 (클라우드 타겟) ```/travelbuddy/flightspecials``` 경로 (온프레미스 소스) 모두 서비스가 정상적으로 동작하는지 확인합니다.
...
### 이제 이를 바탕으로 데이터 마이그레이션을 수행하고, 애플리케이션 기능들을 클라우드 환경으로 전환하는 작업을 수행할 차례입니다.
