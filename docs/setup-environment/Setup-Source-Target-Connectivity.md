...
3. 소스 환경으로 라우팅될 수 있도록 아래와 같이 정보를 입력하고 ```정적 경로 생성 (Create static route)```를 클릭합니다.
   ...
   > 소스 측 환경을 담당하는 분께 확인해 보면 소스 측 VPC의 CIDR 대역은 ```10.10.0.0/16```으로 구성되어 있습니다.<br>
   ...
   > - 즉, `10.0.0.0 ~ 10.15.255.255` 범위는 소스 측 범위로 할당된 것이라면, `10.16.0.0 ~ 10.31.255.255` 범위는 타겟 측 범위로 할당된 것이라고 볼 수 있습니다.
...
2. 온프레미스 소스 환경의 ```VPC > 라우팅 테이블 (Route Tables)```로 이동하여, ```DMSWorkshop-Source-RouteTable (혹은 설정한 이름에 따른 라우팅 테이블)``` 을 선택한 후 다음과 같이 라우팅 경로를 추가합니다 (라우팅 탭).
   ...
   > ❓ 라우팅 경로에서 타겟 측 CIDR 대역 ```10.16.0.0/12```의 의미는?<br>
